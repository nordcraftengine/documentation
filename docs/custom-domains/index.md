...
- `myfeature-{project-name}.nordcraft.site`: each branch has a unique domain in the format `{branch-name}-{project-name}.nordcraft.site`. This is a publicly accessibly URL that you can use to view work in progress in a browser, and share it with people for sign off and approval of new features and functionality.
...
On the Domain Management page, you can see all the domains registered on your project, including the `.nordcraft.site` domains for the `main` branch. You can also add, edit and delete domains.
